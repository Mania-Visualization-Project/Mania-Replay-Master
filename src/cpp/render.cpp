...
int main(int argc, char *arg[]) {
    ...
    Size imageSize(GAME_WIDTH, GAME_HEIGHT);
    VideoWriter writer("output.avi", VideoWriter::fourcc('P', 'I', 'M', '1'), FPS, imageSize);
    Mat image(imageSize, TYPE_IMAGE);
    Mat dark(imageSize, TYPE_IMAGE, Scalar::all(0));
    ifstream beatMap(arg[1]);//"G:\\code\\java\\ManiaReplayMaster\\beat.txt");
    ifstream replayMap(arg[2]);//"G:\\code\\java\\ManiaReplayMaster\\replay.txt");
    vector<Note> beats;
    vector<Note> replay;

    int beatCount, key, replayCount;
    beatMap >> beatCount >> key;
    for (int i = 0; i < beatCount; i++) {
        readNote(beatMap, beats);
    }
    replayMap >> replayCount;
    for (int i = 0; i < beatCount; i++) {
        readNote(replayMap, replay);
    }

    auto duration = beats[beatCount - 1].getEndTime() + 2000;
    cout << duration << endl;
    int lastBeat = 0, lastReplay = 0;
    columnWidth = (int) ((double) GAME_WIDTH / key);
